...
def _():
    ...
    return BigQueryConnector, Counter, mo, pd, re
...
@app.cell
def _(BigQueryConnector, DATASET_ID, PROJECT_ID, TABLE_ID, mo):
    # BigQueryコネクタ初期化
    try:
...
def _(df_clean):
    # 上位エンゲージメント投稿（TOP 20）
    top_engagement = df_clean.nlargest(100, 'total_engagement')[
...
def _(Counter, df_clean, pd, re):
    ...
    hashtag_df
    return all_content, hashtag_df


@app.cell
def _(Counter, all_content, pd, re):
    # メンション抽出
